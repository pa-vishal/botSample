...
const { ciscoSparkVerifyOrgSecurity } = require('./lib/ciscoSparkVerifyOrgSecurity.js');
...
// hello //
///////////
controller.hears(['hello', 'greetings'], 'direct_message,direct_mention', function (bot, message) {
    ciscoSparkVerifyOrgSecurity(bot, message, function (error) {
        if (!error) {
...
// ping TARGET //
/////////////////
controller.hears('pug me', 'direct_message,direct_mention', function (bot, message) {
    ciscoSparkVerifyOrgSecurity(bot, message, function (error) {
        if (!error) {
            const { chatOps_pugMe } = require('./chatOps_pugMe.js');
            chatOps_pugMe(bot, message);
        }
    })
});
////////////////////////////////////////////////////////////////////////////////
        ...
        if (!error) {
            ...
            ciscoSparkGetPersonDetails(message, function (error, person) {
                if (error) {
                    ...
                    let thisReply = "Hello " + message.user + "!\n\n";
                    sparkMessage.push(thisReply);
